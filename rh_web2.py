# 1. Update imports to add request and redirect
from flask import Flask, render_template, jsonify, request, redirect, url_for, session

# 2. Add secret key for session management
app = Flask(__name__)
#app.secret_key = os.environ.get('SECRET_KEY', 'your_default_secret_key')  # Better to use environment variable

# 3. Modify the fetch_and_process_option_orders function to use session credentials
def fetch_and_process_option_orders():
    ...
    try:
        ...
        # If login fails, return a signal that credentials are needed
...
@app.route('/')
def index():
    """Render the main page or login page depending on authentication status"""
    try:
        # Try automated login first (robin_stocks will use saved credentials if available)
        #login = r.login()
...
# 6. Update the API endpoint to check for login
...
def get_options():
    ...
    try:
        ...
        if result and "login_required" in result and result["login_required"]:
